// composables/useFlightSearch.ts
...
export const useFlightSearch = () => {
  ...
  const searchFlights = async (params: SearchParams) => {
    ...
    try {
      ...
      const departureDateStr = useFormatDate(params.departureDate.toISOString(), 'date-send');
      const arrivalDateStr = useFormatDate(params.arrivalDate.toISOString(), 'date-send');
  ...
  const calculateFinalPrice = (basePrice: number, departureDate: string): number => {
  ...
  const statistics = computed(() => {
    return {
      ...
      availableFlights: flights.value.filter(f => f.available_seats > 0).length,
      fullFlights: flights.value.filter(f => f.status === 'full').length,
    };
  });
